...
export const user = pgTable(
	'user',
	{
		...
		baseCurrencyBalance: decimal('base_currency_balance', {
			...
			.notNull()
			.default('100.00000000'), // $100
		bio: varchar('bio', { length: 160 }).default('An profile on Catplay!'),
...
export const coin = pgTable(
	'coin',
	{
		...
		poolCoinAmount: decimal('pool_coin_amount', { precision: 30, scale: 8 })
			.notNull()
			.default('0.00000000'),
		poolBaseCurrencyAmount: decimal('pool_base_currency_amount', { precision: 30, scale: 8 })
			.notNull()
			.default('0.00000000'),
		createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
		updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
...
export const transaction = pgTable(
	'transaction',
	{
		...
		totalBaseCurrencyAmount: decimal('total_base_currency_amount', {
			precision: 30,
			scale: 8
		}).notNull(),
		timestamp: timestamp('timestamp', { withTimezone: true }).notNull().defaultNow(),
		recipientUserId: integer('recipient_user_id').references(() => user.id, {
			onDelete: 'set null'
		}),
...
export const predictionQuestion = pgTable(
	'prediction_question',
	{
		id: serial('id').primaryKey(),
		...
		totalYesAmount: decimal('total_yes_amount', { precision: 30, scale: 8 })
			.notNull()
			.default('0.00000000'),
		totalNoAmount: decimal('total_no_amount', { precision: 30, scale: 8 })
			.notNull()
			.default('0.00000000'),
	...
	(table) => {
		return {
			...
			statusResolutionIdx: index('prediction_question_status_resolution_idx').on(
				table.status,
...
export const predictionBet = pgTable(
	'prediction_bet',
	{
		id: serial('id').primaryKey(),
		userId: integer('user_id').references(() => user.id, { onDelete: 'set null' }),
		questionId: integer('question_id')
			.notNull()
			.references(() => predictionQuestion.id, { onDelete: 'cascade' }),
...
export const accountDeletionRequest = pgTable(
	...
	(table) => {
		return {
			...
			scheduledDeletionIdx: index('account_deletion_request_scheduled_deletion_idx').on(
...
export const battlepassSeason = pgTable('battlepass_season', {
	id: serial('id').primaryKey(),
...
export const battlepassTier = pgTable(
	'battlepass_tier',
	{
		id: serial('id').primaryKey(),
		seasonId: integer('season_id').notNull().references(() => battlepassSeason.id, { onDelete: 'cascade' }),
		level: integer('level').notNull(),
		tier: varchar('tier', { length: 10 }).notNull().default('free'),
		taskDescription: varchar('task_description', { length: 200 }).notNull(),
		taskType: varchar('task_type', { length: 50 }).notNull(),
		taskTarget: integer('task_target').notNull().default(1),
		rewardType: varchar('reward_type', { length: 20 }).notNull(),
		rewardAmount: decimal('reward_amount', { precision: 30, scale: 8 }).notNull().default('0'),
		rewardLabel: varchar('reward_label', { length: 100 })
	},
	(table) => ({
		seasonLevelTierUnique: unique('battlepass_tier_season_level_tier_unique').on(table.seasonId, table.level, table.tier)
	})
);
...
export const battlepassProgress = pgTable(
	'battlepass_progress',
	{
		...
		claimedTiers: text('claimed_tiers').notNull().default('[]'),
		baselineTrades: integer('baseline_trades').notNull().default(0),
		baselineArcade: integer('baseline_arcade').notNull().default(0),
		baselineStreak: integer('baseline_streak').notNull().default(0)
	},
	(table) => ({
		userSeasonUnique: unique('battlepass_progress_unique').on(table.userId, table.seasonId),
		userIdIdx: index('battlepass_progress_user_id_idx').on(table.userId),
		seasonIdIdx: index('battlepass_progress_season_id_idx').on(table.seasonId)
	})
);
...
export const friendship = pgTable(
	...
	(table) => ({
		friendshipUnique: unique('friendship_unique').on(table.requesterId, table.addresseeId),
		requesterIdx: index('friendship_requester_idx').on(table.requesterId),
		addresseeIdx: index('friendship_addressee_idx').on(table.addresseeId),
		statusIdx: index('friendship_status_idx').on(table.status),
		noSelfFriend: check('no_self_friend', sql`requester_id != addressee_id`)
	})
);
...
export const directMessage = pgTable(
	'direct_message',
	{
		...
		recipientId: integer('recipient_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
	...
	(table) => ({
		...
		conversationIdx: index('dm_conversation_idx').on(table.senderId, table.recipientId),
