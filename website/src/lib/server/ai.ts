...
export async function validateQuestion(
	...
	if (coinSymbols.length > 0) {
		const coinData = await Promise.all(coinSymbols.map((symbol) => getCoinData(symbol)));
		...
		if (existingCoins.length > 0 || nonExistentCoins.length > 0) {
			...
			if (existingCoins.length > 0) {
				coinContext += `\nEXISTING: ${existingCoins
					.map((coin) =>
						coin
							? `*${coin.symbol} (${coin.name}): $${coin.currentPrice.toFixed(6)}, Market Cap: $${coin.marketCap.toFixed(2)}, Listed: ${coin.isListed}`
							: 'none'
					)
					.join('\n')}`;
			}
	...
	const prompt = `
You are evaluating whether a prediction market question is valid and answerable for Catplay, a cryptocurrency trading simulation platform.

Question: "${question}"

Current Catplay Market Context:
...
Also determine:
- Whether this question requires web search (external events, real-world data, non-Catplay information)
- If the question is related to the Catplay market, and contains what appears to be a coin name, ensure it's properly formatted (e.g. *BTC, *DOGE). Invalid question example: "will BTC reach $100,000 in 1 hour?" (invalid coin format, should be *BTC). 
...
export async function resolveQuestion(
	...
	const catplayData = customCatplayData || (await getCatplayData(question));

	const prompt = `
You are resolving a prediction market question with a definitive YES or NO answer for Catplay.

Question: "${question}"

Current Catplay Platform Data:
${catplayData}
...
export async function getCatplayData(question?: string): Promise<string> {
	try {
		const marketOverview = await getMarketOverview();
		...
		if (question) {
			...
			if (coinSymbols.length > 0) {
				...
				if (nonExistentCoins.length > 0) {
					coinSpecificData += `\nNON-EXISTENT COINS: ${nonExistentCoins.map((symbol) => `*${symbol}`).join(', ')} - These coins do not exist on the Catplay platform`;
				}
...
${coin.recentTrades
	...
	.join('\n')}`;
		...
		return `
Current Timestamp: ${new Date().toISOString()}
Platform: Catplay - Cryptocurrency Trading Simulation
...
- Coins use *SYMBOL format (e.g., *BTC, *DOGE, *SHIB)${coinSpecificData}
		...
		console.error('Error generating Catplay data:', error);
