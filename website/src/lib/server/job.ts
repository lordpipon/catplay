	...
	try {
		...
		for (const question of expiredQuestions) {
			try {
				...
				const catplayData = await getCatplayData(question.question);
...
\`\`\``;
				...
				if (resolution.confidence < 50) {
					console.log(
						`Cancelling question ${question.id} due to low confidence: ${resolution.confidence}`
					);
					...
					await db.transaction(async (tx) => {
						...
						const bets = await tx
							...
							.from(predictionBet)
							.where(
								and(eq(predictionBet.questionId, question.id), isNull(predictionBet.settledAt))
							);
				...
				await db.transaction(async (tx) => {
					...
					const bets = await tx
						...
						.from(predictionBet)
						.where(and(eq(predictionBet.questionId, question.id), isNull(predictionBet.settledAt)));
	...
	try {
		...
		for (const request of expiredRequests) {
			try {
				...
				console.error(
					`❌ Failed to process account deletion for user ID: ${request.userId}`,
					error
				);
				...
				await db
					...
					.set({
						...
						reason: request.reason
							? `${request.reason} - FAILED: ${error.message}`
