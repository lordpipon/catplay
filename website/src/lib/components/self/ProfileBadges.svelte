<script lang="ts">
	...
	let best = $derived(user.bestTrophy ?? null);
	let seasonTrophies = $derived(
		(user.seasonTrophies ?? []) as Array<{ seasonName: string; rank: number }>
	);
...
<div class="flex items-center gap-1">
	...
	{/if}
	{#if user.halloweenBadge2026}
		<SilentBadge icon="/pumpkin.png" text="Halloween 2026" class="text-primary {badgeClass}" />
	{/if}
	{#if best}
		<Tooltip.Root>
			<Tooltip.Trigger>
				<div class="cursor-pointer rounded-full p-1 opacity-80 hover:opacity-100 {trophyColor} {badgeClass}">
					<HugeiconsIcon icon={ChampionIcon} class="h-4 w-4" />
				</div>
			</Tooltip.Trigger>
			<Tooltip.Content
