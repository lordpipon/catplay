<script lang="ts">
	...
	function handleNavClick(title: string) { setOpenMobile(false); }
	function handleModeToggle() { setMode(mode.current === 'light' ? 'dark' : 'light'); }
	function formatCurrency(value: number): string {
		return value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
	}
	function handleLiveTradesClick() { goto('/live'); setOpenMobile(false); }
	...
	function handleAccountClick() { if ($USER_DATA) { goto(`/user/${$USER_DATA.id}`); setOpenMobile(false); } }
	...
	<Sidebar.Content>
				...
				<Sidebar.GroupContent>
					<div class="px-2 py-1">
						{#if !$PORTFOLIO_SUMMARY}
							<div class="space-y-2"><Skeleton class="h-8 w-full rounded" /></div>
						{:else}
							<DailyRewards />
						{/if}
					</div>
		...
		<Sidebar.Group>
			<Sidebar.GroupLabel class="flex items-center justify-between">
				...
				<button onclick={handleLiveTradesClick} class="text-muted-foreground hover:text-foreground cursor-pointer text-xs transition-colors">
					View All
				</button>
			</Sidebar.GroupLabel>
			<Sidebar.GroupContent>
					...
					{#if $isLoadingTrades}
						{#each Array(5) as _, i}
							<div class="flex items-center gap-2 py-1 text-xs">
								...
								<div class="flex-1"><div class="flex items-center gap-1"><Skeleton class="h-3 w-12" /><Skeleton class="h-3 w-28" /></div></div>
							</div>
						{/each}
					...
					<div class="space-y-2 px-2 py-1">
						{#if !$PORTFOLIO_SUMMARY}
							<div class="flex items-center justify-between">
								<div class="flex items-center gap-2"><Skeleton class="h-4 w-4 rounded" /><Skeleton class="h-4 w-16" /></div>
								<Skeleton class="h-5 w-16 rounded" />
							</div>
							<div class="space-y-1">
								<div class="flex justify-between"><Skeleton class="h-3 w-8" /><Skeleton class="h-3 w-12" /></div>
								<div class="flex justify-between"><Skeleton class="h-3 w-10" /><Skeleton class="h-3 w-12" /></div>
							</div>
						{:else}
							<div class="flex items-center justify-between">
								<div class="flex items-center gap-2">
			...
			<Sidebar.Menu>
				<Sidebar.MenuItem>
					<DropdownMenu.Root>
						...
						<DropdownMenu.Content class="w-(--bits-dropdown-menu-anchor-width) min-w-56 rounded-lg p-2" side={isMobile ? 'bottom' : 'right'} align="end" sideOffset={4}>
							...
							<DropdownMenu.Group>
								<DropdownMenu.Item onclick={handleAccountClick}><HugeiconsIcon icon={UserIcon} />Account</DropdownMenu.Item>
								<DropdownMenu.Item onclick={handleSettingsClick}><HugeiconsIcon icon={Settings01Icon} />Settings</DropdownMenu.Item>
								<DropdownMenu.Item onclick={handlePrestigeClick}><HugeiconsIcon icon={CrownIcon} />Prestige</DropdownMenu.Item>
							</DropdownMenu.Group>
							<DropdownMenu.Separator />
							<DropdownMenu.Group>
								<DropdownMenu.Item onclick={handleAPIClick}><HugeiconsIcon icon={Key01Icon} />API</DropdownMenu.Item>
								<DropdownMenu.Item onclick={() => { showPromoCode = true; setOpenMobile(false); }}><HugeiconsIcon icon={GiftIcon} />Promo code</DropdownMenu.Item>
								<DropdownMenu.Item onclick={handleUserManualClick}><HugeiconsIcon icon={BookOpen01Icon} />User Manual</DropdownMenu.Item>
								<DropdownMenu.Item onclick={handleModeToggle}>
									{#if mode.current === 'light'}<HugeiconsIcon icon={Moon01Icon} />Dark Mode{:else}<HugeiconsIcon icon={Sun01Icon} />Light Mode{/if}
								</DropdownMenu.Item>
							</DropdownMenu.Group>
								...
								<DropdownMenu.Group>
									{#if hasFlag($USER_DATA?.flags, 'IS_HEAD_ADMIN')}
										<DropdownMenu.Item onclick={handleHeadAdminClick} class="text-orange-500 focus:bg-orange-500/10 focus:text-orange-500!">
											<HugeiconsIcon icon={Shield01Icon} class="text-orange-500" />Head Admin Panel
										</DropdownMenu.Item>
									{/if}
									{#if hasFlag($USER_DATA.flags, 'IS_ADMIN', 'IS_HEAD_ADMIN')}
										<DropdownMenu.Item onclick={handleAdminClick} class="text-primary hover:text-primary!"><HugeiconsIcon icon={Shield01Icon} class="text-primary" />Admin Panel</DropdownMenu.Item>
										<DropdownMenu.Item onclick={handleUserManagementClick} class="text-primary hover:text-primary!"><HugeiconsIcon icon={LegalHammerIcon} class="text-primary" />User Management</DropdownMenu.Item>
										<DropdownMenu.Item onclick={handlePromoCodesClick} class="text-primary hover:text-primary!"><HugeiconsIcon icon={Ticket01Icon} class="text-primary" />Manage codes</DropdownMenu.Item>
										<DropdownMenu.Item onclick={handleAdminLogsClick} class="text-primary hover:text-primary!"><HugeiconsIcon icon={Shield01Icon} class="text-primary" />Admin Logs</DropdownMenu.Item>
									{/if}
								</DropdownMenu.Group>
							{/if}
							<DropdownMenu.Separator />
							<DropdownMenu.Group>
								<DropdownMenu.Item onclick={handleTermsClick}><HugeiconsIcon icon={JusticeScale01Icon} />Terms of Service</DropdownMenu.Item>
					...
					<Sidebar.MenuButton>
						{#snippet child({ props }: { props: MenuButtonProps })}
							<a href="/legal/terms" onclick={handleTermsClick} class={`${props.class}`}>
								<HugeiconsIcon icon={JusticeScale01Icon} /><span>Terms of Service</span>
							</a>
						{/snippet}
					</Sidebar.MenuButton>
				</Sidebar.MenuItem>
					...
					<Sidebar.MenuButton>
						{#snippet child({ props }: { props: MenuButtonProps })}
							<a href="/legal/privacy" onclick={handlePrivacyClick} class={`${props.class}`}>
								<HugeiconsIcon icon={ShieldUserIcon} /><span>Privacy Policy</span>
							</a>
						{/snippet}
					</Sidebar.MenuButton>
				</Sidebar.MenuItem>
