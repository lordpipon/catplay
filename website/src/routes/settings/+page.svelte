<script lang="ts">
	...
	let timezone = $state($USER_DATA?.timezone == null ? '' : $USER_DATA.timezone.toString());
	...
	let isDirty = $derived(
		name !== ($USER_DATA?.name || '') ||
			...
			+timezone !== (($USER_DATA?.timezone == null ? '' : $USER_DATA.timezone.toString()))
	);
...
<SEO
	title="Settings - Catplay"
	description="Manage your Catplay account settings, profile information, audio preferences, and privacy options."
					...
					<form onsubmit={handleSubmit} class="space-y-4">
						...
						<div class="space-y-2">
							...
							>
								...
								>
									{timezone === '' ? 'None' : formatTimezone(+timezone)}
