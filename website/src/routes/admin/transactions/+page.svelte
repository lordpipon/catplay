<script lang="ts">
	...
	function resetFilters() {
		...
		goto('/admin/transactions', { noScroll: true, replaceState: true });
		fetchTransactions();
...
<SEO
	title="Admin Transactions - Catplay"
	description="View your complete trading history and transaction records in the Catplay cryptocurrency simulation game."
	noindex={true}
...
<div class="container mx-auto max-w-7xl p-6">
	<header class="mb-8">
		<div class="text-center">
			<h1 class="mb-2 text-3xl font-bold">Admin Transactions</h1>
			<p class="text-muted-foreground mb-6">
				Complete record of trading activity and transactions of the entire platform.
			</p>
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description
				>Complete history of transactions and trading activity based on your filters</Card.Description
			>
		</Card.Header>
		<Card.Content>
			{#if loading}
				...
				<Table.Root>
					...
					<Table.Body>
						{#each transactions as tx (tx.id)}
							...
							{@const rowSender =
								tx.sender ??
								(!tx.isTransfer && (tx.type === 'SELL' || tx.type === 'BURN')
									? tx.traderUsername
									: null)}
							{@const rowRecipient =
								tx.recipient ?? (!tx.isTransfer && tx.type === 'BUY' ? tx.traderUsername : null)}
							<!-- Main row -->
							<Table.Row
							...
							>
								...
								<Table.Cell>
									{#if rowSender}
										<span class="font-medium">{rowSender}</span>
									{:else}
								...
								<Table.Cell>
									{#if rowRecipient}
										<span class="font-medium">{rowRecipient}</span>
									{:else}
										<span class="text-muted-foreground">-</span>
									{/if}
								</Table.Cell>
