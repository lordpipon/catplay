<script lang="ts">
	...
	let customScriptsOpen = $state(false);
	...
	{:else}
		...
		<Card>
			...
			<CardContent class="space-y-4">
				...
				<!-- Top Coins Endpoint -->
				<Collapsible.Root bind:open={customScriptsOpen}>
					<Collapsible.Trigger
						class="hover:bg-muted/50 flex w-full items-center justify-between rounded-lg border p-4"
					>
						<div class="text-left">
							<h3 class="text-lg font-semibold">Custom Scripts & Bots</h3>
							<p class="text-muted-foreground text-sm">Build your own market trackers and tools</p>
						</div>
						{#if customScriptsOpen}
							<HugeiconsIcon icon={ArrowDown01Icon} class="h-4 w-4" />
						{:else}
							<HugeiconsIcon icon={ArrowRight01Icon} class="h-4 w-4" />
						{/if}
					...
					<Collapsible.Content class="space-y-3 p-4">
						<p class="text-muted-foreground text-sm">
							Use the API with any language that can make HTTP requests. All you need is an API key
							(created above) passed as a <code class="bg-muted rounded px-1">Bearer</code> token.
							Below are ready-to-use examples for tracking the market.
						</p>
...
const BASE = 'https://catplay.lordpipon.com';
...
async function watchMarket() {
  const { coins } = await api('/api/v1/market?sortBy=change24h&sortOrder=desc&limit=10');

  console.clear();
  console.log('Top movers right now:');
  for (const c of coins) {
    console.log(
      \`\${c.symbol.padEnd(8)} $\${c.price.toFixed(4).padStart(12)}  \${c.change24h >= 0 ? '+' : ''}\${c.change24h.toFixed(2)}%\`
    );
  }
}

setInterval(watchMarket, 30_000);
watchMarket();`}
								displayOnly={true}
							/>
						</div>
						...
						<div class="space-y-2">
							<h4 class="font-medium">Python — price alert for a specific coin</h4>
							<Codeblock
...
while True:
    ...
    print(f"*{coin['symbol']}: \${price:,.6f}")

    if price > 1.0:
        print("Alert: price crossed $1.00!")
        break
    time.sleep(60)`}
								displayOnly={true}
							/>
...
curl -H "Authorization: Bearer ctpl_yourkey" \\
  "https://catplay.lordpipon.com/api/v1/market?sortBy=change24h&sortOrder=desc&limit=5"

# Coin details with candles
curl -H "Authorization: Bearer ctpl_yourkey" \\
  "https://catplay.lordpipon.com/api/v1/coin/TEST"`}
								displayOnly={true}
							/>
						</div>

						<div class="rounded-lg border bg-muted/30 p-3 text-sm">
							<p class="font-medium">Ideas for scripts</p>
							<ul class="text-muted-foreground mt-1 list-inside list-disc space-y-0.5">
								<li>Discord bot that posts top movers every hour</li>
								<li>Portfolio tracker syncing your holdings into a spreadsheet</li>
								<li>Price alerts via webhooks or push notifications</li>
								<li>Custom dashboards charting candlestick data from /api/v1/coin</li>
							</ul>
							<p class="text-muted-foreground mt-2">
								Rate limit still applies (2,000 requests/day), so poll no faster than every ~45s.
							</p>
						</div>
					</Collapsible.Content>
				</Collapsible.Root>
				...
				<Collapsible.Root bind:open={topCoinsOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							...
							<Codeblock text="GET https://catplay.lordpipon.com/api/v1/top" displayOnly={true} />
						</div>
						<div class="space-y-2">
      ...
      "volume24h": 13744958.18
				...
				<Collapsible.Root bind:open={marketDataOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							<h4 class="font-medium">Endpoint</h4>
							<Codeblock text="GET https://catplay.lordpipon.com/api/v1/market" displayOnly={true} />
						</div>
						<div class="space-y-2">
				...
				<Collapsible.Root bind:open={coinDetailsOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							<h4 class="font-medium">Endpoint</h4>
							<Codeblock
								text="GET https://catplay.lordpipon.com/api/v1/coin/&lbrace;symbol&rbrace;"
								displayOnly={true}
							/>
						</div>
						<div class="space-y-2">
				...
				<Collapsible.Root bind:open={holdersOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							<h4 class="font-medium">Endpoint</h4>
							<Codeblock
								text="GET https://catplay.lordpipon.com/api/v1/holders/&lbrace;symbol&rbrace;"
								displayOnly={true}
							/>
						</div>
						<div class="space-y-2">
      ...
      "liquidationValue": 4368219.41924125
				...
				<Collapsible.Root bind:open={hopiumOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							<h4 class="font-medium">Endpoint</h4>
							<Codeblock text="GET https://catplay.lordpipon.com/api/v1/hopium" displayOnly={true} />
						</div>
						<div class="space-y-2">
							<h4 class="font-medium">Query Parameters</h4>
							<div class="rounded-md border p-3">
  ...
  "questions": [
    {
      "id": 101,
      "question": "will elon musk tweet about catplay?",
      "status": "ACTIVE",
				...
				<Collapsible.Root bind:open={hopiumDetailsOpen}>
					...
					<Collapsible.Content class="space-y-3 p-4">
						...
						<div class="space-y-2">
							<h4 class="font-medium">Endpoint</h4>
							<Codeblock
								text="GET https://catplay.lordpipon.com/api/v1/hopium/&lbrace;question_id&rbrace;"
								displayOnly={true}
							/>
						</div>
						<div class="space-y-2">
