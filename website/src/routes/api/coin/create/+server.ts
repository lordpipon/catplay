...
async function handleIconUpload(iconFile: File | null, symbol: string): Promise<string | null> {
	...
	return await uploadCoinIcon(symbol, new Uint8Array(arrayBuffer), iconFile.type);
}
...
export async function POST({ request }) {
	...
	await db.transaction(async (tx) => {
		...
		if (currentBalance < TOTAL_COST) {
			throw error(
				400,
				`Insufficient funds. You need $${TOTAL_COST.toFixed(2)} but only have $${currentBalance.toFixed(2)}.`
			);
		}
	...
	return json({
		...
		supply: FIXED_SUPPLY,
		message:
			'Coin created! All tokens are in the liquidity pool. Buy some if you want to hold them.'
	});
}
