...
export async function POST({ params, request }) {
	...
	if (!['STAKE', 'UNSTAKE', 'CLAIM'].includes(type))
		throw error(400, 'Invalid staking action type');
	if (
		type !== 'CLAIM' &&
	...
	return await db.transaction(async (tx) => {
		...
		if (type === 'STAKE') {
			const [userHolding] = await tx
				...
				.from(userPortfolio)
				.where(and(eq(userPortfolio.userId, userId), eq(userPortfolio.coinId, coinId)))
				.limit(1);
			...
			await tx
				...
				.set({ totalStaked: (Number(pool.totalStaked) + amount).toString() })
				.where(eq(coinStakingPool.coinId, coinId));
		...
		if (type === 'UNSTAKE') {
			...
			if (existingHolding) {
				await tx
					.update(userPortfolio)
					.set({
						quantity: (Number(existingHolding.quantity) + amount).toString(),
						updatedAt: new Date()
					})
					.where(and(eq(userPortfolio.userId, userId), eq(userPortfolio.coinId, coinId)));
			} else {
		...
		if (type === 'CLAIM') {
			...
			if (existingHolding) {
				await tx
					.update(userPortfolio)
					.set({
						quantity: (Number(existingHolding.quantity) + currentClaimable).toString(),
						updatedAt: new Date()
					})
					.where(and(eq(userPortfolio.userId, userId), eq(userPortfolio.coinId, coinId)));
			} else {
