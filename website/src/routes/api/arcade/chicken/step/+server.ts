...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		if (game.laneCars[game.currentLane]) {
			const laneCars = game.laneCars;
			...
			return json({
				hitCar: true,
				carLane: game.currentLane,
				allCarLanes: laneCars,
				newBalance: balance,
				status: 'lost',
		...
		game.currentLane += 1;
		game.currentMultiplier = chk_multiplier(game.currentLane, game.difficulty);

		if (game.currentLane >= game.laneCars.length) {
			const allCarLanes = game.laneCars;
		...
		console.error('Chicken step error:', e);
