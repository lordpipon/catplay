...
<SEO
	title={season ? `${season.name} — Catplay` : 'Season — Catplay'}
	description="Catplay season standings. Everyone starts from the same stake."
/>
	...
	{:else}
		...
		<Card.Root class="relative isolate overflow-hidden">
			...
			<Card.Header class="relative -mt-6 pt-6 pb-4">
				<div class="bg-card/40 pointer-events-none absolute -inset-x-6 -top-6 -z-[5] bottom-[-1.25rem] backdrop-blur-md" aria-hidden="true"></div>
							...
							{#each leaderboard as entry (entry.userId)}
								...
								>
									<span class="flex w-7 shrink-0 justify-center">
										{#if entry.rank === 1}
											<HugeiconsIcon icon={MedalFirstPlaceIcon} class="h-4 w-4 text-yellow-500" />
										{:else if entry.rank === 2}
											<HugeiconsIcon icon={MedalSecondPlaceIcon} class="h-4 w-4 text-slate-400" />
										{:else if entry.rank === 3}
											<HugeiconsIcon icon={MedalThirdPlaceIcon} class="h-4 w-4 text-amber-700" />
										{:else}
									...
									<HoverCard.Content class="w-80" side="top" sideOffset={14}>
					...
					{#each data.pastSeasons as past (past.number)}
						<a
							...
							season.number === past.number
								...
								: ''}"
						>
							<HugeiconsIcon icon={Award05Icon} class="text-muted-foreground h-4 w-4 shrink-0" />
							<span class="min-w-0 flex-1 truncate text-sm font-medium">{past.name}</span>
							<span class="text-muted-foreground shrink-0 text-xs">
								{formatDate(past.endedAt)}
							</span>
							<HugeiconsIcon icon={ArrowRight01Icon} class="text-muted-foreground h-4 w-4 shrink-0" />
						</a>
					{/each}
				</Card.Content>
			</Card.Root>
